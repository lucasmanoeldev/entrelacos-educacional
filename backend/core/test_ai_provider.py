...
from .ai_provider import generate_draft, GenerationError, GROQ_ENDPOINT


@override_settings(GROQ_API_KEY='test-secret', GROQ_MODEL='openai/gpt-oss-20b')
class GROQProviderTests(SimpleTestCase):
    ...
    def reply(self, content, finish='stop'):
        return httpx.Response(200, request=httpx.Request('POST', GROQ_ENDPOINT), json={
    ...
    def test_correct_groq_endpoint_model_and_private_header(self, call):
        ...
        self.assertEqual(kwargs['json']['max_completion_tokens'], 1024)
    ...
    def test_provider_errors_do_not_leak_body_or_key(self, call):
        for status in [400, 401, 403, 404, 429, 500]:
            call.return_value = httpx.Response(status, request=httpx.Request('POST', GROQ_ENDPOINT), text='test-secret internal-error')
            with self.assertRaises(GenerationError) as error:
                generate_draft(self.parameters)
