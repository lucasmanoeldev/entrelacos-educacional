...
export async function mount({root,signal}:Context){
  ...
  root.addEventListener('click',async event=>{
    ...
    else if(action==='add-question'&&data.questions.length<50)data.questions.push(blank());
