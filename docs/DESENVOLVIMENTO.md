...
```text
frontend/     Astro 7 + TypeScript nativo (sem React) → Vercel ou Render Static Site
backend/      Django 5.2 LTS + Django REST Framework → Render Web Service
PostgreSQL    Supabase (SQLite para desenvolvimento e testes locais)
```
...
Requisitos: **Node.js 22.12+ e Python 3.12**. Execute os comandos na raiz deste projeto.
...
Veja **[docs/DEPLOY.md](DEPLOY.md)** para os passos completos, variáveis e solução de erros.
...
Defina `GROQ_API_KEY` **somente na API**, nunca em variáveis `PUBLIC_*`. `GROQ_MODEL` usa `openai/gpt-oss-20b`. Veja [ativação GROQ](GROQ.md). A chamada usa saída JSON e validação das perguntas; o professor revisa antes de publicar. Sem chave, a API responde com uma mensagem clara e a criação manual segue disponível.
...
Detalhes da API: **[docs/API.md](API.md)**.
